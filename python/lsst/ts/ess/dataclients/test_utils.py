...
check_reply_func_dict = {
    dataclients.SensorType.CSAT3B: "check_csat3b_reply",
    dataclients.SensorType.EFM100C: "check_efm100c_reply",
    dataclients.SensorType.HX85A: "check_hx85a_reply",
    dataclients.SensorType.HX85BA: "check_hx85ba_reply",
    dataclients.SensorType.LD250: "check_ld250_reply",
    dataclients.SensorType.TEMPERATURE: "check_temperature_reply",
    dataclients.SensorType.WINDSONIC: "check_windsonic_reply",
}
...
class MockTestTools:
    def __init__(self) -> None:
        self.reply: None | dict[str, dataclients.TelemetryDataType] = None

    async def _callback(self, reply: dict[str, dataclients.TelemetryDataType]) -> None:
        self.reply = reply
    ...
    async def check_mock_device(
        self,
        sensor_type: dataclients.SensorType,
        num_channels: int = 0,
        ...
        sensor_class = dataclients.sensor.sensor_registry[sensor_type]
        ...
        async with dataclients.device.MockDevice(
            name="MockSensor",
            ...
            func_arg_values = {
                "reply": self.reply[dataclients.Key.TELEMETRY],
                "name": "MockSensor",
            ...
            await self.wait_for_reply()
            assert self.reply is not None
            func_arg_values["reply"] = self.reply[dataclients.Key.TELEMETRY]
            if "missed_channels" in func_args.args:
                func_arg_values["missed_channels"] = missed_channels
            func(**func_arg_values)
    ...
    def check_csat3b_reply(
        ...
        assert time > 0
        if in_error_state:
            assert dataclients.ResponseCode.DEVICE_READ_ERROR == response_code
        else:
            assert dataclients.ResponseCode.OK == response_code
        ...
        for i in range(0, 4):
            if i < missed_channels or in_error_state:
                ...
                if i in [0, 1, 2]:
                    assert dataclients.device.MockWindSpeedConfig.min <= resp[i]
                    assert resp[i] <= dataclients.device.MockWindSpeedConfig.max
                elif i == 3:
                    assert dataclients.device.MockTemperatureConfig.min <= resp[i]
                    assert resp[i] <= dataclients.device.MockTemperatureConfig.max
    ...
    def check_ld250_reply(
        ...
        time = float(reply["timestamp"])
        response_code = reply["response_code"]
        resp: dataclients.TelemetryDataType = []
        for value in reply["sensor_telemetry"]:
            assert (
        ...
        if resp[0] == dataclients.LD250TelemetryPrefix.NOISE_PREFIX:
            # Check noise response.
            assert len(resp) == 1
        elif resp[0] == dataclients.LD250TelemetryPrefix.STATUS_PREFIX:
            # Check status response.
            assert len(resp) == 6
            assert dataclients.device.MockStrikeRateConfig.min <= resp[1]
            assert resp[1] <= dataclients.device.MockStrikeRateConfig.max
            assert dataclients.device.MockStrikeRateConfig.min <= resp[2]
            assert resp[2] <= dataclients.device.MockStrikeRateConfig.max
            assert resp[3] in [0, 1]
            assert resp[4] in [0, 1]
            assert dataclients.device.MockAzimuthConfig.min <= resp[5]
            assert resp[5] <= dataclients.device.MockAzimuthConfig.max
        elif resp[0] == dataclients.LD250TelemetryPrefix.STRIKE_PREFIX:
            # Check strike response.
            assert len(resp) == 4
            assert dataclients.device.MockDistanceConfig.min <= resp[1]
            assert resp[1] <= dataclients.device.MockDistanceConfig.max
            assert dataclients.device.MockDistanceConfig.min <= resp[2]
            assert resp[2] <= dataclients.device.MockDistanceConfig.max
            assert dataclients.device.MockAzimuthConfig.min <= resp[3]
            assert resp[3] <= dataclients.device.MockAzimuthConfig.max
    ...
    def check_efm100c_reply(
        ...
        assert dataclients.device.MockElectricFieldStrengthConfig.min <= resp[0]
        assert resp[0] <= dataclients.device.MockElectricFieldStrengthConfig.max
    ...
    def check_hx85a_reply(
        ...
        for i in range(0, 3):
            if i < missed_channels or in_error_state:
                ...
                if i == 0:
                    assert dataclients.device.MockHumidityConfig.min <= resp[i]
                    assert resp[i] <= dataclients.device.MockHumidityConfig.max
                elif i == 1:
                    assert dataclients.device.MockTemperatureConfig.min <= resp[i]
                    assert resp[i] <= dataclients.device.MockTemperatureConfig.max
                else:
                    assert dataclients.device.MockDewPointConfig.min <= resp[i]
                    assert resp[i] <= dataclients.device.MockDewPointConfig.max
        ...
        for i in range(0, 3):
            if i < missed_channels or in_error_state:
                ...
                if i == 0:
                    ...
                    assert dataclients.device.MockPressureConfig.min <= resp[i]
                    assert resp[i] <= dataclients.device.MockPressureConfig.max
    ...
    def check_windsonic_reply(
        ...
        if in_error_state:
            ...
            assert dataclients.ResponseCode.OK == response_code
        assert dataclients.device.MockWindSpeedConfig.min <= resp[0]
        assert resp[0] <= dataclients.device.MockWindSpeedConfig.max
        assert dataclients.device.MockDirectionConfig.min <= resp[1]
        assert resp[1] <= dataclients.device.MockDirectionConfig.max
