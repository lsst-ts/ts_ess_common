...
class ControllerDataClient(BaseReadLoopDataClient):
    ...
    def __init__(
        ...
        self.telemetry_dispatch_dict = self.get_telemetry_dispatch_dict()
            ...
            """
{
  ...
  "additionalProperties": false
    ...
    async def connect(self) -> None:
        ...
        await asyncio.wait_for(self.client.start_task, timeout=CONNECT_TIMEOUT)
    ...
    async def _basic_run_command(self, data: dict[Key, Any]) -> None:
    ...
    async def process_telemetry(
        ...
        ------
        RuntimeError
            If the response code is ResponseCode.DEVICE_READ_ERROR
        """
        try:
            ...
            if response_code == ResponseCode.OK:
                telemetry_method = self.telemetry_dispatch_dict.get(
                    SensorType(device_configuration.sens_type)
                )
