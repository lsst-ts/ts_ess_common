...
class BaseReadLoopDataClient(abc.ABC):
            ...
            try:
                ...
                if self.num_consecutive_read_timeouts >= self.max_num_reconnects:
                    self.log.error(
                        ...
                        f">= {self.max_num_reconnects=}; giving up."
                    )
                ...
                message = (
                    ...
                    f"of {self.max_num_reconnects} allowed."
                )
                ...
                if self.loop_should_end:
                    self.log.warning("Stopping so not raising the exception.")
                else:
                    raise
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            self.log.warning("Stopping so ignoring.")
