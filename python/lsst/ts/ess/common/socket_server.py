...
from .constants import Command, JsonKeys
...
class SocketServer(tcpip.OneClientReadLoopServer):
    ...
    async def read_and_dispatch(self) -> None:
        ...
        cmd = items[JsonKeys.COMMAND]
        kwargs = items[JsonKeys.PARAMETERS]
        if cmd == Command.EXIT:
            await self.exit()
    ...
    async def close_client(self, cancel_read_loop_task: bool = True) -> None:
        ...
        await super().close_client(cancel_read_loop_task=cancel_read_loop_task)
